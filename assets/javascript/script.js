...
app_key_nutrition = 'd5e1c45fe92fe6ff4bfa429206a54311';api_id_calculate = '63e18821';
app_key_calculate = '834af81f9aa84aa05cecc3d1497f0065';function opensection(section){
    btn1 = document.getElementById("nutritionanalysisbtn");
    btn2 = document.getElementById("nutritioncalculatorbtn");    btn1.style.display = "none";
    btn2.style.display = "none";    if(section == 1){
    ...
    let formatedfood = food.replace(/ /g, '%20');    fat_value =  document.getElementById("fatid")
    ...
    .then(function(data){
        ...
        foodName = data.ingredients[0].parsed[0].food;        foodQuantity = data.ingredients[0].parsed[0].quantity;
        foodUnit = data.ingredients[0].parsed[0].measure;
        foodType = data.ingredients[0].parsed[0].food;
        foodWeight = data.ingredients[0].parsed[0].weight;        // newRow = "<tr><td>" + foodQuantity +"</td><td>"+ foodUnit +"</td><td>"+ foodType +"</td><td>" + calories + "</td><td>"+ foodWeight +"</td></tr>"
        // tableBody.append(newRow);        // bindinding to html
        ...
        // <button value="1">Click Me</button>        cal_value.innerText = calories;
    ...
    .then(function(data){
        ...
        for(i = 0; i < arrayName.length; i++){
            ...
            foodProteins = Math.round(data.parsed[i].food.nutrients.PROCNT);            // console.log(foodProteins);            foodDetails += "<li><button>Qty : 1</button><button>Food : "+ foodName +"</button> <div><p>Energy: "+ foodEnergy +" kcal</p><p>Protein: "+ foodProteins +" g</p><p>Fat: "+ foodFats +" g</p><p>Carbs: "+ foodFats +" g</p></div></li>"
        }
